...
It follows the HLT, which only runs on the events accepted by the L1T, and has access to the full detector readout, operating with an average processing-time budget of the order of \SI{1}{\second} per event, in Phase~2.
...
Such an approach enables a significant reduction of trigger thresholds with respect to the standard data stream, and thus the exploration of phase-space regions otherwise inaccessible, at the cost of the resolution degradation of physics objects. \par
%
The High-Luminosity LHC (HL-LHC) is scheduled to start taking data in 2030, reaching unprecedented instantaneous luminosities of \sm\SI{7.5e34}{\per\square\centi\meter\per\second}, corresponding to a \num{\sim 3.5} times increase over peak Run~3 conditions, and an average pileup of 200, compared to \num{\sim 60} in Run~3.
...
\caption{Schematic view of a quarter of the Phase-2 CMS Tracking Detector in a r versus z projection.
...
Simultaneously, its efficiency in the endcaps remains compatible with the baseline within 5\%, while in the barrel, especially near the transition regions ($\vert{}\eta\vert{} \approx 1.5$), both \textit{Patatrack}-based configurations achieve up to $\sim$10\% higher efficiency.
On top, for very high $|\eta|$ values, the two \textit{Patatrack} configurations achieve a higher efficiency.
It should be noted that these measurements do not consider the strip section of the OT, neither the pixel-strip modules in the endcaps.
...
\caption{Average processing time spent per event in the different HLT reconstruction components of the NGT Scouting stream, for the \texttt{Legacy}, \texttt{Patatrack}, and \texttt{Extended Patatrack} configurations, running on CPU only (left) or with GPU offloading enabled (right).
  The CPU-only \texttt{Legacy} result is identical in both panels, since this configuration includes no heterogeneous implementation.
  The \texttt{Extended Patatrack} configuration yields an 18\% speed-up relative to the \texttt{Patatrack} configuration in the average processing time per event for the CPU-only setup -- already including the additional cost of incorporating OT macro-pixel hits -- owing to the use of pixel-only tracks and the removal of both the second iteration and the CKF step.
  The same configuration reaches a 34\% speed-up relative to the \texttt{Patatrack} configuration when GPUs are enabled, for identical reasons, on top of the 8\% higher throughput of the \textit{Patatrack} algorithm on GPUs, rendering the pixel-tracking step essentially negligible in terms of processing time.
  Taken from~\cite{ngtdps}.
}
\label{fig:timing}
\end{figure}
%
...
Firstly, we show that removing muon seeds does not impact physics performance, simplifying the reconstruction chain.
...
Finally, the IO single iteration is overhauled: the \texttt{Legacy} pixel tracks are replaced by the pixel tracks created with the \texttt{Extended Patatrack} configuration.
...
%
In this context, the efficiency is defined as the fraction of simulated target muons that are matched to a reconstructed muon object, where a matching is considered if the two objects share more than 75\% of their hits.
The fake rate is instead defined as the ratio of reconstructed muons not matched to any simulated target muon over the total number of reconstructed muons.
Note that only a subset of the simulated muons is considered.
In particular, simulated muons must satisfy $|\eta| \leq 2.4$ and $p_\text{T} > 0.9$ GeV, and must come from the signal event.
In Fig.~\ref{fig:muonperf} we show the performance of the new single iteration IO step, in terms of reconstruction efficiency and fake rate, when compared to the legacy, two-iteration IO approach.
...
While showcasing a clear time reduction, we present promising pixel tracking performance results, competitive with the ones obtained with full tracking.
...
\bibitem{patatrack}
  A. Bocci et al. ``Heterogeneous Reconstruction of Tracks and Primary Vertices With the CMS Pixel Tracker''. Frontiers in Big Data Volume 3 - 2020 (2020). issn: 2624-909X. DOI: \url{10.3389/fdata.2020.601728}. url: \url{https://arxiv.org/abs/2008.13461}.
%  
% Format for books
% \bibitem{RefB}
