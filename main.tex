...
The resulting scouting output is a reduced analysis-ready data format which by-passes the offline reconstruction.
These strategies enable a significant reduction of trigger thresholds with respect to the standard data stream, and thus the exploration of phase-space regions otherwise inaccessible, at the cost of the resolution degradation of physics objects. \par
%
...
The reconstruction of tracker muon tracks runs two steps: the Inside-Out (IO) and the Outside-In (OI).
...
On top, instead of being recomputed, extended pixel tracks selected within a given seed region are simply recycled from the ones already computed for tracking purposes.
...
%
The performance improvements brought by the updates to the muon tracking were measured in simulated $\text{Z}\rightarrow\mu\mu$ events under HL-LHC conditions, with an average pileup of 200.
...
The muons are reconstructed by merging the IO and OI muon track collections, ie., also taking into accout the OI recovery step.
Only simulated muons produced in the Z decay within the detector acceptance conditions are considered.
In particular, simulated muons must satisfy $|\eta| \leq 2.4$ and $p_\text{T} > 0.9$ GeV.
